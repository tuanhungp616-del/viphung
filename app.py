...
# ==========================================
# 🔐 HỆ THỐNG AN NINH QUỐC PHÒNG (ĐÃ ĐỔI KEY)
# ==========================================
KEYS_DB = {
    "hungadmin67": "admin",
    "viphung": "user",
...
def get_id(item):
    ...
    matches = re.findall(r"'?(?:id|phien|referenceId|sessionId|matchId)'?\s*:\s*'?'?(\d+)'?'?", str(item), re.IGNORECASE)
    return int(matches[0]) if matches else 0

# ==================================================
# 🧠 LÕI 1: THUẬT TOÁN ĐA VŨ TRỤ OMNIVERSE (CHO MD5)
# ==================================================
...
# ==================================================
# 🧠 LÕI 2: THUẬT TOÁN MONTE CARLO (CHO SÀN THƯỜNG)
# ==================================================
def monte_carlo_predict(is_chanle, current_history):
    if len(current_history) < 5: return "TÀI", 53.5, "Đang nạp dữ liệu..."
...
def scan_game():
    ...
    try:
        ...
        if not isinstance(lst, list):
            ...
            return jsonify({"status": "success", "data": {"du_doan": "TÀI", "ti_le": 58.5, "loi_khuyen": "API TRỰC TIẾP", "phien": str(phien_thuc)}})
